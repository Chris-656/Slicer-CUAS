...
class HistogramLabelWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    parametersFormLayout.addRow("Input Scalar Volume: ", self.inputVolumeSelector)
    ...
    #self.inputLabelSelector.addAttribute( "vtkMRMLLabelMapVolumeNode", "LabelMap", 0 )
    self.inputLabelSelector.selectNodeUponCreation = False
    ...
    self.inputLabelSelector.setToolTip( "Select the Label Volume for masking the histograms." )
    ...
    self.outputSelector.setToolTip( "Create a masked volume from label volume" )
    parametersFormLayout.addRow("Output Volume (Masked): ", self.outputSelector)

    # input density plot
    self.densityTab = qt.QWidget()
    self.densityLayout = qt.QGridLayout(self.densityTab)
    self.density = qt.QCheckBox()
    self.density.toolTip = "When checked, create normalized histogram"
    self.density.checked = False
    self.densityLayout.addWidget(self.density, 0, 0)
    parametersFormLayout.addRow("Normalized Histogram: ", self.density)

    # input cumulative plot
    self.cumulativeHist = qt.QWidget()
    #self.densityLayout = qt.QGridLayout(self.densityTab)
    self.cumulativeHist = qt.QCheckBox()
    self.cumulativeHist.toolTip = "When checked, create cumulative histogram"
    ...
    self.applyButton.toolTip = "Start"
    ...
    self.plotSelector.connect("currentNodeChanged(vtkMRMLNode*)", self.onSelect)
    self.seriesSelector.connect("currentNodeChanged(vtkMRMLNode*)", self.onSelect)
  ...
  def onSelect(self):
    self.applyButton.enabled =  self.inputVolumeSelector.currentNode() and self.plotSelector.currentNode() and self.seriesSelector.currentNode()
    #self.applyButton.enabled =  False
  ...
  def onApplyButton(self):
    ...
    logic.run(self.inputVolumeSelector.currentNode(),
        self.inputLabelSelector.currentNode(),
        self.BinCount.value,
...
class HistogramLabelLogic(ScriptedLoadableModuleLogic):
  ...
  def createMaskedVolume(self,inputVolume, inputLabel, maskedValue, outputVolume):
    mif = sitk.MaskImageFilter()
    mif.SetOutsideValue(maskedValue)

    roiSITK  = sitkUtils.PullVolumeFromSlicer(inputVolume)
    maskSITK = sitkUtils.PullVolumeFromSlicer(inputLabel)

    final = mif.Execute(roiSITK,maskSITK)
    sitkUtils.PushVolumeToSlicer(final,outputVolume)

  def calcHistogram(self, inputVolume, inputLabel, binCount, useDensity, useCumulative):
    # Compute histogram values
    #
    volume = slicer.util.arrayFromVolume(inputVolume)

    if (inputLabel):
        label = slicer.util.arrayFromVolume(inputLabel)
  ...
  def createPlot(self,histogram,plotChart,seriesNode):
    # Save results to a new table node in order to have the possibility to save the data
  ...
  def run(self, inputVolume, inputLabel, binCount, useDensity, useCumulative,outputVolume, plotChart, series):
    """
    Run the actual algorithm
    """
    print('Startet !!!')
    mskVolumeLabel = 1
    if not self.isValidInputOutputData(inputVolume,inputLabel,series, plotChart,outputVolume):
      slicer.util.errorDisplay('Check the input')
      return False

    print('Trying to calc Histogram')
    ...
    #
    if (outputVolume):
        self.createMaskedVolume(inputVolume, inputLabel, 0, outputVolume)
    ...
    # Compute histogram values
    #
    histogram = self.calcHistogram(inputVolume, inputLabel, binCount, useDensity, useCumulative)
